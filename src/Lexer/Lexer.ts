...
class Lexer {
  ...
  private static parseToken(): Token | null {
    ...
    if (token === "(")
      return new Token(TokenType.OPEN_PAREN_TOKEN, token, this.line);
    if (token === ")")
      return new Token(TokenType.CLOSE_PAREN_TOKEN, token, this.line);
    if (token === "[")
      return new Token(TokenType.OPEN_BRACKET_TOKEN, token, this.line);
    if (token === "]")
    ...
    if (token === ";")
      return new Token(TokenType.SEMICOLON_TOKEN, token, this.line);
    if (token === ":")
      return new Token(TokenType.COLON_TOKEN, token, this.line);
    if (token === "?")
  ...
  static parseNumberLiteralToken(token: string): Token | null {
    ...
    if (isNaN(parsedNumber))
      throw new LexerError(`Invalid number literal: '${number}'`, this.line);
  ...
  private static consume(): string {
    if (this.tokens.length === 0)
      throw new LexerError("Unexpected EOF", this.line);
