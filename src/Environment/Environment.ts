...
class Environment {
  ...
  public defineVariable(
    ...
    if (value.type === ValueType.CUSTOM) {
      ...
      if (!this.isDefinedType(type))
        throw new EnvironmentError(`Type ${type} is not defined!`);
    }
  ...
  public isDefinedVariable(name: string): boolean {
    return (
      this.variables.has(name) || Boolean(this.parent?.isDefinedVariable(name))
    );
  }
  ...
  public isDefinedFunction(name: string): boolean {
    return (
      this.functions.has(name) || Boolean(this.parent?.isDefinedFunction(name))
    );
  }

  public getVariable(name: string): [RuntimeValue, string, boolean] {
    if (!this.isDefinedVariable(name))
      throw new EnvironmentError(`Variable ${name} is not defined!`);
    return (
      this.variables.get(name) ||
      (this.parent?.getVariable(name) as [RuntimeValue, string, boolean])
    );
  }

  public getType(name: string): any {
    if (!this.isDefinedType(name))
      throw new EnvironmentError(`Type ${name} is not defined!`);
  ...
  public setVariable(name: string, value: RuntimeValue): void {
    if (!this.isDefinedVariable(name))
      throw new EnvironmentError(`Variable ${name} is not defined!`);
    if (this.isDefinedFunction(name))
      throw new EnvironmentError(`Variable ${name} is a function!`);
    if (this.isDefinedType(name))
      throw new EnvironmentError(`Variable ${name} is a type!`);
